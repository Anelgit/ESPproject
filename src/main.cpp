 ...
 *******************************************************/
 ...
 #define ON_MIN_FRAMES 60            // ON must be at least ~0.6 s, otherwise treat as wake
         ...
         if (vad_rms < VAD_OFF) {
           ...
           if (now - silence_since >= SILENCE_HANG_MS) {
             ...
             if (cls >= 0 && (now - last_trigger_ms[cls] >= CLASS_COOLDOWN_MS)) {
               ...
               // If this “ON” utterance is very short, it’s probably the wakeword being mis-bucketed.
                 if (seg_len < ON_MIN_FRAMES) {
                   Serial.println("ready");        // treat as wake, don't toggle
                   #if KWS_DEBUG
